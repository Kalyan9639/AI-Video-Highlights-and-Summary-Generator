...
from moviepy.editor import (
    ...
    CompositeVideoClip,  # <-- ADD THIS
)
import moviepy.video.fx.all as vfx
from typing import Optional, Union

"""
This file contains a function to create a "summary video" (reel) by
extracting and combining clips from a longer video based on
a JSON timestamp file, with randomized transitions.
...
def _parse_time_to_seconds(
    time_input: Optional[Union[float, str, int]]
) -> Optional[float]:
    """
    ...
    if isinstance(time_input, str):
        try:
            ...
            print(
                f"Warning: Could not parse time string '{time_input}'. Defaulting to 0."
            )
            return 0.0
...
def create_summary_video(
    video_path: str, json_path: str, output_path: str = "summary_video.mp4"
) -> None:
    """
    Creates a summary video by extracting and concatenating clips
    from a source video based on timestamps in a JSON file.

    This version adds randomized transitions between clips for a
    social media reel.
    ...
    try:
        ...
        # --- 4. Extract Clips (No effects yet) ---
        print("Extracting raw clips based on timestamps...")
        for i, item in enumerate(timestamps):
            ...
            if start_t >= end_t:
                print(
                    f"Warning: Skipping item {i+1}, start time ({start_str}) is after end time ({end_str})."
                )
                continue

            if end_t > main_video.duration:
                print(
                    f"Warning: Item {i+1} end time ({end_str}) exceeds video duration. Clipping to end."
                )
        ...
        # --- 5. Build Final Video with Randomized Transitions ---
        print(f"\nBuilding final video with {len(clips_list)} clips...")
        
        transition_duration = 0.5  # 0.5 seconds is perfect for reels
        transition_types = ["crossfade", "dip_to_black", "slide"]
        slide_directions = ["left", "right", "top", "bottom"]

        # Start with the first clip as our "final" clip
        final_clip = clips_list[0]

        # Iteratively join the remaining clips
        for next_clip in clips_list[1:]:
            transition = random.choice(transition_types)
            print(f"  ...adding clip with '{transition}' transition")

            if transition == "crossfade":
                # This transition is a "composite" and overlaps the clips.
                # It also crossfades the audio automatically.
                final_clip = CompositeVideoClip(
                    [
                        final_clip,
                        next_clip.set_start(
                            final_clip.duration - transition_duration
                        ),
                    ]
                )

            elif transition == "slide":
                # This is a slide from/to black.
                direction = random.choice(slide_directions)
                final_clip = concatenate_videoclips(
                    [
                        final_clip.fx(vfx.slideout, transition_duration, direction),
                        next_clip.fx(vfx.slidein, transition_duration, direction),
                    ]
                )

            else:  # "dip_to_black"
                # This is a simple fade-out then fade-in.
                # This also fades the audio in and out.
                final_clip = concatenate_videoclips(
                    [
                        final_clip.fadeout(transition_duration),
                        next_clip.fadein(transition_duration),
                    ]
                )

        # --- 6. Write Final Video ---
        print(f"\nWriting final summary video to {output_path}...")
        
        # --- ADDED: Speed up the final clip ---
        print("Applying 1.5x speed to the final reel...")
        ...
        final_clip.write_videofile(
            ...
            fps=(main_video.fps or 24) * 1.5,  # Adjust FPS for speed
        )
...
if __name__ == "__main__":
    ...
    MY_JSON_FILE = "timestamps_reel.json"  # The file you provided
    ...
    if os.path.exists(MY_LONG_VIDEO) and os.path.exists(MY_JSON_FILE):
        create_summary_video(
            ...
            output_path="my_final_reel_with_transitions.mp4",
        )
    else:
